...
class RecoveryNode(private val ctx: NodeContext? = null) : GameNode {
    ...
    override suspend fun execute(ctx: GameContext): GamePhase? {
        ...
        try {
            ...
            // 1. 结算弹窗 — 黑色遮罩 + 弹窗
            if (nodeCtx.detector.matchTemplate(screen, ScreenState.SETTLEMENT_POPUP) != null) {
                nodeCtx.log("恢复: 检测到结算弹窗，切入结算节点")
                return GamePhase.SETTLEMENT
            }

            // 2. 结算确认按钮
            if (nodeCtx.detector.matchTemplate(screen, ScreenState.CONFIRM_BUTTON) != null) {
                nodeCtx.log("恢复: 检测到确认按钮，切入结算节点")
                return GamePhase.SETTLEMENT
            }

            // 3. 队伍房间 — 准备按钮
            if (nodeCtx.detector.matchTemplate(screen, ScreenState.READY_BUTTON) != null) {
                nodeCtx.log("恢复: 检测到准备按钮，切入悬赏详情")
                return GamePhase.BOUNTY_DETAIL
            }

            // 4. 战斗加载界面
            if (nodeCtx.detector.matchTemplate(screen, ScreenState.BATTLE_LOADING) != null) {
                nodeCtx.log("恢复: 检测到战斗加载，切入加载节点")
                return GamePhase.BATTLE_LOADING
            }

            // 5. 战斗中 — 滑铲按钮
            if (nodeCtx.detector.matchTemplate(screen, ScreenState.SLIDE_BUTTON) != null) {
                nodeCtx.log("恢复: 检测到战斗界面，切入战斗节点")
                return GamePhase.FIGHT
            }

            // 6. 战斗中 — 跳跃按钮
            if (nodeCtx.detector.matchTemplate(screen, ScreenState.JUMP_BUTTON) != null) {
                nodeCtx.log("恢复: 检测到战斗界面，切入战斗节点")
                return GamePhase.FIGHT
            }

            // 7. 个人悬赏列表
            if (nodeCtx.detector.matchTemplate(screen, ScreenState.PERSONAL_BOUNTY_LIST_SCREEN) != null) {
                nodeCtx.log("恢复: 检测到个人悬赏列表，切入个人悬赏中心")
                return GamePhase.PERSONAL_BOUNTY_CENTER
            }

            // 8. 招募列表 — 团队招募标签
            if (nodeCtx.detector.matchTemplate(screen, ScreenState.RECRUIT_LIST_SCREEN) != null) {
                nodeCtx.log("恢复: 检测到招募列表页面，直接切入")
                return GamePhase.RECRUIT_LIST
            }

            // 9. 大厅 — 聊天按钮（左上角）
            if (nodeCtx.detector.matchTemplate(screen, ScreenState.CHAT_ICON) != null) {
