...
| ASSIST_BUTTON | DEFEAT | 左下 1/3（cropBottomLeftThird） | 观战面板标识 |
...
| 助战按钮 | `detector.cropBottomLeftThird(mat)` | 宽33% x 高33%（左下） | DefeatNode |
...
截图 → 识别当前页面（使用裁剪匹配）:
  ...
  → DEFEAT_SCREEN / DEFEAT_BACK_BUTTON / ASSIST_BUTTON → DEFEAT
