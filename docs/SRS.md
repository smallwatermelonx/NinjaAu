...
```
IDLE → LOBBY → BOUNTY_LIST → BOUNTY_DETAIL → BATTLE_LOADING → FIGHT → SETTLEMENT → LOBBY（循环）
                                              ↓                      ↓
                                            RECOVERY ←←←←←←←←←←←←←←←
```
...
| IDLE | - | 初始状态，等待启动 |
| LOBBY | HallNode | 大厅导航 |
| BOUNTY_LIST | BountyListNode | 招募列表扫描 |
...
详见 `app/src/main/java/com/example/ninjaau/model/ScreenState.kt`，共29种屏幕状态。
